...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for actionCheck in self.all_actions:
            nodeA = PgNode_a(actionCheck)
            if nodeA.prenodes.issubset(self.s_levels[level]):
                self.a_levels[level].add(nodeA)
                for preNode in nodeA.prenodes:
                    preNode.children.add(nodeA)
                    preNode.parents.add(nodeA)
    ...
    def add_literal_level(self, level):
        ...
        for levelA in self.a_levels[level - 1]:
            for nodeA in levelA.effnodes:
                self.s_levels[level].add(nodeA)
                levelA.children.add(nodeA)
                nodeA.parents.add(levelA)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        for actionEffect in node_a1.action.effect_add:
            if actionEffect in node_a2.action.effect_rem:
                return True

        for actionEffect in node_a2.action.effect_add:
            if actionEffect in node_a1.action.effect_rem:
                return True
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        """
        for actionEffect in node_a1.action.effect_add:
            if actionEffect in node_a2.action.precond_neg:
                return True

        for actionEffect in node_a2.action.effect_add:
            if actionEffect in node_a1.action.precond_neg:
                return True

        for actionEffect in node_a1.action.effect_rem:
            if actionEffect in node_a2.action.precond_pos:
                return True

        for actionEffect in node_a2.action.effect_rem:
            if actionEffect in node_a1.action.precond_pos:
                return True
    ...
    def h_levelsum(self) -> int:
        ...
        allGoals = self.problem.goal.copy()

        while len(allGoals) > 0:
            for levelS in range(len(self.s_levels)):
                symDecid = [state.symbol for smth, state in enumerate(self.s_levels[levelS]) if state.is_pos]

                for goalToSum in allGoals:
                    if goalToSum in symDecid:
                        level_sum += levelS
                        allGoals.remove(goalToSum)
